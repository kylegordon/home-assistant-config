...
- **Frigate** — trigger on MQTT topic `frigate/events` and filter on `trigger.payload_json`: `type` (`new` / `update` / `end`), `after.label`, `after.camera`. See `packages/outside_lights.yaml`. Note that `after.camera` is always a *camera*, never a Frigate zone; zone occupancy is a subset of its parent camera's, so trigger on the camera and read zone membership from `after.entered_zones` / `after.current_zones` (compare with `before.entered_zones` for the edge, and drop `after.stationary` objects: a parked car is reacquired all day). Snapshot images come from the `frigate/<camera>/<object>/snapshot` MQTT cameras in `mqtt.yaml`.
